...
app = Flask(__name__)
# Allow CORS for the API endpoint globally or specifically
...
def api_analyze():
    ...
    if not pipeline.client:
        return jsonify({"error": "Groq API key not configured. Please set your API key first."}), 400
...
def api_set_key():
    """Accept a Groq API key from the frontend and configure the pipeline."""
    data = request.json
    key = (data or {}).get('api_key', '').strip()
    if not key:
        return jsonify({"error": "No API key provided."}), 400
    try:
        pipeline.configure_api(key)
...
def api_health():
    """Health check — tells the frontend if the backend is alive and if an API key is set."""
    return jsonify({
        "status": "ok",
        "api_key_configured": pipeline.client is not None,
        "model_hierarchy": [m["name"] for m in pipeline.get_model_hierarchy()],
    })

@app.route('/api/models', methods=['GET'])
def api_models():
    """Return the full model fallback hierarchy."""
    return jsonify({
        "models": pipeline.get_model_hierarchy(),
        "last_used": pipeline.last_model_used,
    })
